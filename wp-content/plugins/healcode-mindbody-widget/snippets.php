...
<div>
	<form method="post">
		<fieldset
			...
			<legend><h3>MINDBODY Widgets</h3></legend>
<?php
			global $wpdb;
			...
			<h1></h1>
			<div class="hc-shortcode-copy">
				After you create a widget, add it to your website pages with the MINDBODY button on the 'Edit Page' screen or copy the shortcode
				below and paste it where you want it to appear in your page.
			</div>
			<input  id="submit_hmw"
...
?>
						<td style="text-align: center;"><a
							href='<?php echo admin_url('admin.php?page=insert-html-short-manage&action=snippet-status&snippetId='.$entry->id.'&status=1&pageno='.$pagenum); ?>'>Activate
						</a>
						</td>
<?php
								}elseif ($entry->status == 1){
?>
						...
						<td style="text-align: center;"><a
							href='<?php echo admin_url('admin.php?page=insert-html-short-manage&action=snippet-status&snippetId='.$entry->id.'&status=2&pageno='.$pagenum); ?>'>
							Deactivate
						</a>
						</td>
<?php
			...
			<input id="submit_hmw"
				...
				name="textFieldButton2" value="Add New Widget"
				 onClick='document.location.href="<?php echo admin_url('admin.php?page=insert-html-short-manage&action=snippet-add');?>"'>
					...
					<div class="hc-empty-shortcode-copy">
						<div class="empty-copy">it's looking empty in here - lets fix that</div>
						<input id="submit_hmw"
							style="cursor: pointer; padding: 15px;" type="button"
							name="textFieldButton2" value="Add New Widget"
							 onClick='document.location.href="<?php echo admin_url('admin.php?page=insert-html-short-manage&action=snippet-add');?>"'>
					</div>
					<?php
